 ...
 * enforce.
 *
 * conventional-types.json and the workflow hold the same list in two
 * languages, nothing makes them agree, and both drifts are silent: a gate
 * missing a type red-lights a title that would have released correctly, and a
 * gate carrying an extra one lets a commit onto master that the changelog then
 * omits without a word. Hence a test rather than a comment.
 */
